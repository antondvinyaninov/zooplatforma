...
export default function OrganizationDetailPage() {
  ...
  const loadOrganizationData = async () => {
    try {
      ...
      const orgRes = await fetch(`http://localhost:8000/api/organizations/${orgId}`, {
        credentials: 'include',
      });
      ...
      const membersRes = await fetch(`http://localhost:8000/api/organizations/${orgId}/members`, {
        credentials: 'include',
      });
      ...
      const [petsRes, postsRes] = await Promise.all([
        fetch(`http://localhost:8000/api/pets?organization_id=${orgId}`, { credentials: 'include' }),
        fetch(`http://localhost:8000/api/posts?organization_id=${orgId}`, { credentials: 'include' }),
      ]);
