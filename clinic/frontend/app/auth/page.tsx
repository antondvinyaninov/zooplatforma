  ...
  const checkAuth = async () => {
    try {
      ...
      if (!meResponse.ok) {
        ...
        if (from !== 'main') {
          window.location.href = 'http://localhost:3000/auth?redirect=http://localhost:6300/auth?from=main';
        } else {
          setChecking(false);
        }
        return;
      ...
      if (meResult.success) {
        // Уже авторизован - переходим к выбору клиники
        router.push('/select');
        return;
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50">
        <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full text-center">
          ...
          <button
            onClick={() => window.location.href = 'http://localhost:3000/auth?redirect=http://localhost:6300'}
