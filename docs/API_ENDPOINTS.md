...
```
GET    /api/users                  Получить всех пользователей (возвращает: id, name, last_name, email, avatar, created_at)
GET    /api/users/:id              Получить пользователя по ID (возвращает: id, name, last_name, email, avatar, bio, location, phone, created_at)
PUT    /api/profile                Обновить свой профиль
GET    /api/profile/:id            Получить профиль пользователя
```

**Изменения в v0.3.4:**
- ✅ `GET /api/users/:id` теперь возвращает полную информацию: `last_name`, `avatar`, `bio`, `location`, `phone`
- ✅ `GET /api/users` теперь возвращает `last_name` и `avatar` для всех пользователей
...
GET    /api/posts/pet/:id          Получить посты, где упоминается питомец
```

**Изменения в v0.3.5:**
- ✅ `GET /api/posts/pet/:id` - новый endpoint для получения всех постов с конкретным питомцем
...
```
GET    /api/pets/user/:id          Получить питомцев пользователя (возвращает: id, user_id, name, species, photo, created_at)
POST   /api/pets                   Добавить питомца (создаёт в PetBase через API)
DELETE /api/pets/:id               Удалить питомца
```

**Изменения в v0.3.4:**
- ✅ `GET /api/pets/user/:id` - исправлен endpoint для загрузки питомцев конкретного пользователя
- ✅ Используется в компоненте CreatePost для прикрепления питомцев к постам
...
### События (история питомца) - 🔒 Планируется v1.1.0

```
GET    /api/pets/:id/events            История событий питомца
POST   /api/pets/:id/events            Добавить событие
